...
0 10 * * 2,4 cd $PROJECT_DIR && /usr/bin/node scripts/auto-generate-blog-post.mjs >> $LOG_FILE 2>&1 && /usr/bin/docker compose -f docker-compose.yml up -d --build app >> $LOG_FILE 2>&1 $CRON_MARK
CRON
